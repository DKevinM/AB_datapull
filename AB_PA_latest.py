...
def assess_pm_quality(pm_raw, pm_corr, a, b, method, humidity=None):
    ...
    # Use corrected value for ceiling checks if available
    ...
    if value > 1500:
        ...
        # Keep on map if you want to show possible extreme smoke,
        # but it will still be clearly flagged.
        use_for_map = True
    ...
    try:
        ...
        for _, row in df_result.iterrows():
            ...
            record = {
                ...
                "pm_corrected": float(row["pm_corr_original"]) if "pm_corr_original" in row and pd.notna(row["pm_corr_original"]) else (float(pm_corr) if pd.notna(pm_corr) else None),
