# PA_MB_pull.py
# Pull all PurpleAir sensors in Manitoba and save as CSV
...
# 1) Load Manitoba boundary (can be any provincial polygon)
mb = gpd.read_file("data/Manitoba.shp")

# Make sure it’s in WGS84 (lat/lon) for the API bbox
if mb.crs is None or mb.crs.to_epsg() != 4326:
    mb = mb.to_crs(epsg=4326)

# 2) Get bounding box [minx, miny, maxx, maxy]
minx, miny, maxx, maxy = mb.total_bounds
...
# 6) Clip to Manitoba polygon (so we don’t keep SK/ON/ND border sensors)
mb_union = mb.geometry.union_all()   # single polygon for the province

inside = gdf[gdf.geometry.intersects(mb_union)].copy()
inside["province"] = "MB"
...
inside_live.to_csv("data/MB_PA_sensors.csv", index=False)

print(f"Total sensors from API: {len(gdf)}")
print(f"Sensors inside Manitoba: {len(inside)}")
